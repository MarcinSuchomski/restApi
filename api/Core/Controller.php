        ...
        function validateJsonInput($input, $requireFields)
        {
            ...
            foreach ($requireFields as $field )
            {
                ...
                if(isset($input['email_address']))
                    if(!$this->isValidEmail($input['email_address'])) return $this->response(false,"Invalid Email","400");
                // check int
                if(isset($input['state_id']))
                    if(!$this->isValidIntager($input['state_id'])) return $this->response(false,"state_id must be a intager","400");

                if(isset($input['fields_types_id']))
                    if(!$this->checkIfExist("fields_types","fields_types_id" , $input['fields_types_id'])) return $this->response(false,"this type of file does not exist ","400");;

                if(isset($input['subscribers_id']))
                    if(!$this->checkIfExist("subscribers" ,"subscribers_id", $input['subscribers_id'])) return $this->response(false,"this subscriber does does not exist ","400");;
